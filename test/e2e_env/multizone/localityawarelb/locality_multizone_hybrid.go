...
func ExternalServicesWithLocalityAwareLb() {
	...
	BeforeAll(func() {
		...
		println(fmt.Sprintf("TEST_LOG: %s", env.KubeZone1.GetZoneEgressPortForward().ApiServerEndpoint))
		println(fmt.Sprintf("TEST_LOG: %s", env.KubeZone1.GetZoneEgressEnvoyTunnel()))
	})
